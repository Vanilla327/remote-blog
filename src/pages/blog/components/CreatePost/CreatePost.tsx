...
export default function CreatePost() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingPost) {
      ...
      //khi ta submit se add thêm id cho formData
      const formDataWithId = { ...formData }
      //dispatch action addPost để thêm post vào trong redux
      dispatch(addPost(formDataWithId))
    }
    //sau khi dispatch ta sẽ clear data bằng cach
    ...
    <form onSubmit={handleSubmit} onReset={handleCancelEditingPost}>
      <div className='mb-6'>
        <label
          ...
          onClick={() => dispatch({ type: 'blog/clicks' })}
        >
          Title
        </label>
        <input
